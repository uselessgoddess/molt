...
pub trait Field<const N: usize> {
    fn field_le<T: Word, const AT: usize>(&self) -> T;
...
impl<const N: usize> Field<N> for [u8; N] {
    fn field_le<T: Word, const AT: usize>(&self) -> T {
        ...
        // The assertion above is the bound check, so the width handed over is
        // the width asked for and the branch below cannot be taken. It is a
        // constant comparison, and the optimiser drops it with the panic.
...
pub fn records_mut<const N: usize>(bytes: &mut [u8]) -> impl Iterator<Item = &mut [u8; N]> {
    let mut rest = bytes;
    core::iter::from_fn(move || {
        let (record, tail) = core::mem::take(&mut rest).split_first_chunk_mut::<N>()?;
        rest = tail;
        Some(record)
    })
}
...
mod tests {
    ...
    fn reads_each_word_width() {
        ...
        assert_eq!(bytes.field_le::<u16, 1>(), 0x0302);
        assert_eq!(bytes.field_le::<u32, 1>(), 0x0504_0302);
        assert_eq!(bytes.field_le::<u64, 0>(), 0x0807_0605_0403_0201);
    }
    ...
    fn reads_both_byte_orders() {
        ...
        assert_eq!(bytes.field_le::<u32, 0>(), 0x7856_3412);
    ...
    /// A field at the very end of its window is in bounds, and the one after it
    /// does not compile. The rejection cannot be asserted from inside the
    /// crate, which is the point of it being a build error.
    #[test]
    fn the_last_field_fits() {
        let mut bytes = [0; 8];

        bytes.put_le::<u32, 4>(0x1234_5678);

        assert_eq!(bytes.field_le::<u32, 4>(), 0x1234_5678);
        assert_eq!(bytes, [0, 0, 0, 0, 0x78, 0x56, 0x34, 0x12]);
    }

    /// Windows come from slices, and that is the step that is allowed to fail.
    #[test]
    fn a_window_is_narrowed_once() {
        let bytes = [1, 2, 3, 4, 5, 6];

        let head: Option<&[u8; 4]> = bytes.first_chunk();
        assert_eq!(head.map(Field::field_le::<u32, 0>), Some(0x0403_0201));

        let short: &[u8] = &bytes[..3];
        assert_eq!(short.first_chunk::<4>(), None);
    }

    /// A table yields whole records and stops, tail and all.
    #[test]
    fn a_table_walks_whole_records() {
        let bytes = [1, 0, 2, 0, 3];

        let mut table = super::records::<2>(&bytes);

        assert_eq!(table.next().map(Field::field_le::<u16, 0>), Some(1));
        assert_eq!(table.next().map(Field::field_le::<u16, 0>), Some(2));
        assert!(table.next().is_none(), "the odd byte at the end was read as a record");
    }

    #[test]
    fn a_table_is_written_record_at_a_time() {
        let mut bytes = [0; 5];
