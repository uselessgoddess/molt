//! Address space identifiers: the tag a TLB keeps two views of one address
//! apart by, and what it costs when the hardware runs out of them.
//!
//! Two domains are two views of the same global address space, so their
//! translations collide by design and the tag is what stops one domain's cached
//! entry from answering the other's load. The field is narrow — sixteen bits at
//! most on RV64, twelve on x86-64, and a machine may implement fewer.
//!
//! Tags are spent by *domains*, not by mappings: a grant or a revoke costs a
//! shootdown ([`va::Epoch`](crate::va::Epoch)) and no tag, so the budget is how
//! many domains exist at once rather than how busy they are.
//!
//! Running out bumps a generation instead of hunting for a free number, the way
//! Linux does on arm64: every tag from the previous generation stops being
//! [`live`](Asids::live) at once, for one whole-TLB flush per wrap.
...
///
/// After a wrap the number alone says nothing about who owns the entries cached
/// under it, so the generation is what makes a stale tag detectable.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Asid {
    value: u16,
    generation: u64,
...
impl Asids {
    /// The tag space of a hart that implements `width` tag bits. Zero is a real
    /// answer: RISC-V allows `ASIDLEN` to be zero, and such a hart shares one
    /// tag between everybody.
    pub const fn new(width: u32) -> Self {
        Self { width: if width > 16 { 16 } else { width }, next: 1, generation: 1, rollovers: 0 }
    }

    /// How many domains can hold a tag between two wraps. Tag zero is the
    /// kernel's, so sixteen bits is 65 535 domains and nine bits is 511.
    ...
    pub fn assign(&mut self) -> Grant {
        if self.next > self.capacity() {
            // Nothing searched for, nothing reclaimed: moving the generation
            // retires every outstanding tag at once.
            self.generation += 1;
            self.rollovers += 1;
            // A hart with no tag bits has only the kernel's number to give, so
            // it gives that one and pays the flush every time.
    ...
    /// Whether a tag still means what it did when it was handed out, or a wrap
    /// has retired it since.
    pub const fn live(&self, asid: Asid) -> bool {
        asid.generation == self.generation
    }

    /// How many wraps, which is how many whole-TLB flushes the tag space cost.
...
mod tests {
    ...
    #[test]
    fn sixteen_bit_field_holds_65535_domains() {
    ...
    fn overwide_width_clamped_to_field() {
        assert_eq!(Asids::new(64).capacity(), 65_535, "a bad probe widened the tag field");
    }
}
