...
use crate::{FRAME_SIZE, FrameAllocator, align_up};
...
// SAFETY: a `Region` is a unique handle to a range of frames, so moving it
// between threads moves the whole claim. It is deliberately not `Sync`: two
// threads holding `&Region` could race writes to one DMA buffer.
...
pub struct Arena<'slots> {
    table: FrameTable<'slots>,
    frames: Frames,
    offset: u64,
    next: u64,
}

impl<'slots> Arena<'slots> {
    ...
    pub fn region(&mut self, bytes: u64) -> Result<Region, DmaError> {
        ...
        let step = align_up(bytes, FRAME_SIZE).ok_or(DmaError::OutOfSpace)?;
...
mod tests {
    ...
    /// A memory map made of a fixed slice of regions.
    ...
    #[test]
    fn a_region_addresses_the_same_bytes_two_ways() {
    ...
    #[test]
    fn a_bulk_copy_round_trips_through_the_region() {
    ...
    #[test]
    fn an_access_past_the_region_is_refused() {
        let mut buffer = [0u8; 16];
    ...
    #[test]
    fn an_arena_hands_out_disjoint_frame_regions() {
    ...
    #[test]
    fn an_arena_refuses_a_region_past_its_span() {
    ...
    #[test]
    fn a_span_broken_by_a_gap_is_refused() {
        // Two one-frame regions with a frame-sized hole between them: the
        // allocator jumps the gap, so a two-frame claim cannot be contiguous.
        let regions = [
    ...
    #[test]
    fn reset_returns_the_span_to_its_table() {
