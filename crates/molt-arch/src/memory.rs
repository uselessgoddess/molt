...
use crate::{FRAME_SIZE, MappingError, MemoryMap, MemoryRegionKind};
...
pub struct FrameTable<'slots> {
    base: Span,
    slots: &'slots mut [Option<Owner>],
}

impl<'slots> FrameTable<'slots> {
    ...
    pub fn over(base: Span, slots: &'slots mut [Option<Owner>]) -> Result<Self, Error> {
    ...
    fn range(&self, span: Span) -> Result<core::ops::Range<usize>, Error> {
...
#[derive(Clone, Copy)]
pub struct Inventory<'map> {
    map: &'map dyn MemoryMap,
    image: Option<Span>,
}

impl<'map> Inventory<'map> {
