...
//! `pci-host-ecam-generic` node in the device tree on RISC-V. Both produce the
//! same three facts, so those live here, beside the memory map they came from.
//!
//! Everything that then *interprets* configuration space — headers, BARs,
//! capabilities, MSI-X — is the `molt-pci` crate's business and depends on this
