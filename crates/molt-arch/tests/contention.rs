...
impl Machine {
    ...
    /// The shootdown protocol as a core reaches it: answer what is open, then
    /// start a round for the batch if nobody else has.
    ///
    /// A round closes only once all eight cores have been through, so a freed
    /// address waits for every core that could hold a translation — the
    /// property, and the way this test livelocks if one is not coming.
