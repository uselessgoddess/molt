...
fn usable_ranges_are_aligned_inward() {
    ...
    assert_eq!(
        ...
        [(0x4000, 0x6000)],
        "a range never claims a byte outside the region the firmware reported"
    );
}
...
fn usable_ranges_start_above_the_floor() {
    ...
    assert_eq!(
        ...
        "the loaded image below the floor stays out of free RAM"
    );
}
...
fn allocated_frames_lie_in_mapped_ranges() {
    ...
    assert!(
        ...
        "the allocator handed out a frame paging would not have mapped"
    );
}
