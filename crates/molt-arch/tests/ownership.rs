...
#[test]
fn an_overlapping_claim_takes_nothing() {
    let mut slots = [None; 4];
    let mut table = table(&mut slots);
...
#[test]
fn released_frames_return_to_the_pool() {
    let mut slots = [None; 4];
    let mut table = table(&mut slots);
    let span = Span::frames(0x4000, 2).unwrap();
...
#[test]
fn frames_from_another_table_are_rejected() {
    let mut mine = [None; 4];
...
#[test]
fn a_claim_outside_the_table_is_rejected() {
...
#[test]
fn a_table_smaller_than_its_span_is_rejected() {
    let mut slots = [None; 2];

    let short = FrameTable::over(Span::frames(0x4000, 3).unwrap(), &mut slots);

    assert!(short.is_err(), "a table that covers less than it claims reports free frames as free");
}
...
#[test]
fn writable_executable_rights_are_rejected() {
