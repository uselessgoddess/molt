...
/// A space with `holes` free-list slots per class.
///
/// The slots are leaked rather than borrowed from the caller: in the kernel they
/// are a static the space outlives (`kernel/src/space.rs`), and a test that
/// declared the array itself would spend two lines saying so.
fn space(holes: usize) -> Space<'static> {
    let slots = vec![Hole::EMPTY; holes].leak();
    Space::over(SV57, slots).expect("a fifty-seven bit space cuts into class arenas")
}

#[test]
fn classes_hand_out_own_alignment() -> Result<(), Error> {
    let mut space = space(12);
...
#[test]
fn hundred_gigabytes_is_one_extent() -> Result<(), Error> {
    let mut space = space(12);
    let log = 100 * (1 << 30);
...
fn already_free_range_refused() -> Result<(), Error> {
    let mut mine = space(12);
    let mut theirs = space(12);
