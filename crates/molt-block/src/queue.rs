...
/// Runs one operation against `disk`, buffer and all.
...
mod tests {
    ...
    fn blocking_disk_takes_one() -> Result<(), BlockError> {
        let bytes = [0; BLOCK];
        let mut queue = Serial::new(Loopback::new(&bytes)?);
    ...
    fn deep_device_answers_newest_first() -> Result<(), BlockError> {
        ...
        bytes[BLOCK] = 7;
        let mut queue = Queued::<_, 2>::new(Loopback::new(&bytes)?);
    ...
    fn slot_frees_when_answer_is_taken() -> Result<(), BlockError> {
        ...
        assert!(queue.start(RequestId::new(1), read(0)).is_ok(), "the slot did not come back");
        Ok(())
    }
}
