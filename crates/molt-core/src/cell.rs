...
///
/// Typed messages keep normal in-image calls on Rust's checked ABI. Dynamic
/// replacement will require a separate, versioned C-compatible boundary.
...
impl<C: Cell> Supervisor<C> {
    ...
    /// Replaces both the component and its owned state in one operation.
