//! Interrupt arrivals a task can await, without a lock.
//!
//! A line is a counter, not a slot: [`raise`](InterruptSlab::raise) increments
//! it and wakes the parked task; [`wait`](InterruptSlab::wait) reports how many
//! arrivals happened since the owner last looked. Two interrupts between two
//! polls are reported as two, so nothing is silently coalesced away.
//!
//! [`bind`](InterruptSlab::bind) claims a line and
//! [`release`](InterruptSlab::release) bumps its generation. A device left
//! programmed with a released vector keeps writing it, but the old token no
//! longer matches; the next owner starts from a fresh baseline.
...
pub struct InterruptFuture<'s, const N: usize> {
    slab: &'s InterruptSlab<N>,
