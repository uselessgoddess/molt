...
// SAFETY: safe construction yields exactly one producer and one consumer. A
...
impl<Op> Submission<Op> {
    pub const fn new(id: RequestId, operation: Op) -> Self {
        Self { id, operation }
    }
...
impl<Op, C, const N: usize> IoRing<Op, C, N> {
    pub const fn new() -> Self {
        Self {
            submissions: SpscRing::new(),
            completions: SpscRing::new(),
        }
    }
