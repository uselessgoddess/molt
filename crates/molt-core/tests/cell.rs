...
#[test]
fn a_cell_that_reported_in_time_is_left_alone() -> Result<(), Fault> {
    let mut supervisor = Supervisor::<Counter>::new(4)?;
...
#[test]
fn a_missed_deadline_restarts_the_cell() -> Result<(), Fault> {
    let events = Arc::new(Mutex::new(Vec::new()));
    let mut supervisor = Supervisor::<Counter>::new(4)?;
    let mut hooks = Hooks(events.clone());
    supervisor.record_heartbeat(10);
...
#[test]
fn a_watchdog_restart_starts_the_next_deadline() -> Result<(), Fault> {
    let mut supervisor = Supervisor::<Counter>::new(4)?;
...
#[test]
fn a_cell_that_will_not_come_back_is_asked_once_per_deadline() -> Result<(), Fault> {
    let mut supervisor = Supervisor::<Fragile>::new(true)?;
