...
impl<T> DerefMut for Unique<T> {
    fn deref_mut(&mut self) -> &mut T {
        // A `Unique` is made from a fresh allocation and gives itself up to
        // share, so there is never a second handle while one exists.
