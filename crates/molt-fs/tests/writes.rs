//! What a write leaves behind for the reads after it.
//!
//! Extents are keyed on the byte they end at, so a read descends onto the one
//! covering its offset instead of walking every write the file ever took. Over
//! a loopback that shows up as fetches: the tree is deeper than the node cache
//! is wide, so a walk of the whole index has to go back to the device and a
//! seek does not.
...
fn read_skips_the_writes_before_it() -> Result<(), FsError> {
    ...
    let disk = Counted { disk: Loopback::writable(&mut bytes)?, reads: Rc::clone(&reads) };
