...
impl<'a> Packet<'a> {
    ...
    pub const fn new(src: Ipv4Addr, dst: Ipv4Addr, protocol: u8, payload: &'a [u8]) -> Self {
        Self { src, dst, protocol, payload }
    }

    pub fn parse(bytes: &'a [u8]) -> Result<Self, NetError> {
        if bytes.len() < Self::HEADER || bytes[0] >> 4 != 4 {
            return Err(NetError::Malformed);
        }
        let header = ((bytes[0] & 0x0f) as usize) * 4;
        if header < Self::HEADER || bytes.len() < header {
            return Err(NetError::Malformed);
        }
    ...
    pub fn emit(&self, bytes: &mut [u8]) -> Result<usize, NetError> {
        let total = Self::HEADER.checked_add(self.payload.len()).ok_or(NetError::Buffer)?;
        ...
        bytes[Self::HEADER..total as usize].copy_from_slice(self.payload);
        Ok(total as usize)
    }
