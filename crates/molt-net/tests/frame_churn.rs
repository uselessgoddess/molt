...
//!
//! Nothing here trusts a length field, so nothing here may read past the bytes
//! the driver actually has. Two shapes of input reach that: frames generated
//! whole, which are mostly refused and prove the refusals, and one bit turned
//! over in a frame this stack emitted itself, which is mostly accepted and
//! proves what acceptance then does.
...
/// Big enough for either header and a payload, small enough that a generated
/// length field points inside it about as often as not.
...
/// be worth generating. Longer than any frame here, and every packet is refused
/// for the same reason.
