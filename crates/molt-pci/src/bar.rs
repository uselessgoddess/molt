...
pub enum BarKind {
    ...
    /// A port-I/O window. Nothing in molt maps one — there is no such thing as
    /// a mapped I/O port — but a device that asks for one should not be
    /// mistaken for a device that asks for nothing.
    Io,
}
...
impl Bar {
    ...
    ///
    /// Not a [`Span`] because a BAR may be smaller than a frame, and inventing
    /// alignment here would hide that from the caller.
...
/// Turns a register pair into a window, or `None` for a BAR the function does
/// not implement.
///
/// `probed` is what the register reads back after all-ones were written to it:
/// a one there is an address bit the device decodes, and the lowest such bit
/// is the window's size. `upper` carries the same pair for the second half of
/// a 64-bit BAR.
fn decode(index: u8, original: u32, probed: u32, upper: Option<(u32, u32)>) -> Option<Bar> {
    ...
    // The length is the value of the lowest writable bit. Truncating to the
    // register's width matters for a 32-bit BAR, whose upper half is not a
    // register at all and must not become 4 GiB of length.
    let len = match bits {
