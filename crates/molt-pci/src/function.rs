...
impl Command {
    ...
    ///
    /// Until the kernel programs an IOMMU this grants the device the whole
    /// physical address space. Nothing in this crate sets it implicitly.
    pub const BUS_MASTER: Self = Self(1 << 2);
    /// Suppress the legacy pin-based interrupt.
    ///
    /// Required alongside MSI or MSI-X: a device left able to assert INTx can
    /// deliver an interrupt through a path nothing is listening on.
...
impl<'window> Function<'window> {
    ...
    pub fn probe(window: Mmio<'window>, address: Address) -> Result<Option<Self>, PciError> {
    ...
    pub const fn window(&self) -> &Mmio<'window> {
        &self.window
    }
    ...
    ///
    /// Takes `&mut self` because enabling decode changes what every other
    /// access in this crate means, so it cannot be done through a shared
    /// handle while a BAR is being sized.
    ...
    pub fn capabilities(&self) -> Result<Capabilities<'_, 'window>, PciError> {
