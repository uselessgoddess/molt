use molt_net::address::{IpAddr, Ipv4Addr, Ipv6Addr};
...
fn ipv6_requires_checksum() {
    ...
    let destination = IpAddr::V6(Ipv6Addr::UNSPECIFIED);
    let bytes = [0, 7, 0, 8, 0, 8, 0, 0];

    assert_eq!(Datagram::parse(source, destination, &bytes), Err(UdpError::Checksum));
}
