...
mod tests {
    ...
    #[test]
    fn a_push_chains_its_segments_and_publishes_the_head() {
    ...
    #[test]
    fn a_full_queue_refuses_the_next_chain() {
        let (mut d, mut a, mut u) = ([0u8; 64], [0u8; 16], [0u8; 64]);
        let mut queue = queue(&mut d, &mut a, &mut u);
    ...
    #[test]
    fn a_pop_returns_the_chains_descriptors_to_the_free_list() {
        let (mut d, mut a, mut u) = ([0u8; 64], [0u8; 16], [0u8; 64]);
        let mut queue = queue(&mut d, &mut a, &mut u);
        let head = queue
            .push(&[Segment::readable(0xaa00, 16), Segment::writable(0xbb00, 512)])
        ...
        // The device completes the chain: element zero names the head, then the
        // used index advances past it.
    ...
    #[test]
    fn region_sizes_match_the_helpers() {
