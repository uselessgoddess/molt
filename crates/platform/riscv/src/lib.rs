...
"#
    ...
    impl RiscVMemoryMap {
        ...
        ///
        /// A hardcoded QEMU `virt` default made `-m` a lie in both directions:
        /// more RAM went unused, less had the frame allocator hand out
        /// addresses that decode to nothing. [`RAM_END`] is now only what a
        /// hart entered without a tree falls back to.
        fn new(device_tree: usize) -> Self {
            ...
            // The image is inside the bank that matters, which is what makes it
            // identifiable on a board with several.
    ...
    /// `sfence.vma` with both operands zero: every prior page-table store
    /// ordered against every translation this hart cached, for every address and
    /// every ASID.
    ///
    /// The whole-hart form rather than the per-address `sfence.vma a0, x0`,
    /// because what is freed is up to a hundred gigabyte leaves and invalidating
    /// them one address at a time costs more than the refill.
