...
//! beyond the image; reserved ranges and device holes stay unmapped so the
//! live-table audit can reject stray leaves.
//!
//! All of that lives below 512 GiB, so the tree is built three levels deep and
//! then rooted as deep as the hart allows ([`enable`]). The width that buys is
//! not for the kernel but for the domains `docs/address-space.md` spends it on.
...
pub fn init(boot_info: &BootInfo<'_>) -> Result<(), PlatformError> {
    ...
    // `.bss` and the boot stack sit inside the data section, which is why it runs
    // to the end of the image rather than to a symbol of its own.
    for (section, start, end) in [
        (ImageSection::Text, bound!(__text_start), bound!(__text_end)),
        (ImageSection::Rodata, bound!(__rodata_start), bound!(__rodata_end)),
        (ImageSection::Data, bound!(__data_start), bound!(__kernel_end)),
    ] {
        map_section(root, &mut frames, &mut log, section, start, end)?;
    }

    for range in UsableRegions::above(boot_info.memory_map(), kernel_end) {
        let start = usize::try_from(range.start()).map_err(|_| address_error())?;
        let end = usize::try_from(range.end()).map_err(|_| address_error())?;
...
///
/// The tree below does not change: every mapping [`init`] made is inside the
/// first 512 GiB, so a wider mode needs only a root whose first entry points at
/// the root below it. Two frames buy the option on both.
///
/// The probe is free, because the privileged specification made it so:
///
/// > If `satp` is written with an unsupported MODE, the entire write has no
/// > effect; no fields in `satp` are modified.
///
/// So the widest write that reads back is the widest mode implemented, and a
/// failed attempt leaves the hart exactly where it was.
///
/// # Safety
///
...
///
/// Write ones into all sixteen, read back which stayed: the width is
/// UNSPECIFIED and the field WARL, so there is no other way. The root does not
/// move, so both writes translate through the same tree and the hart can always
/// fetch its next instruction; what does move is the tag those translations are
/// cached under, hence the flush around each write.
///
/// # Safety
///
...
fn map_section(
    ...
    let aligned_end = align_up(end, PAGE_4K).ok_or_else(address_error)? as u64;
...
/// span at its physical address without mapping anything. The move is permanent:
/// the frames are the caller's for good, and a [`FrameCursor`] read before this
/// call is stale.
...
/// stack and every device window are absent because they were never put in —
/// not because something removed them afterwards.
...
///
/// The entry above the new table is replaced by one aligned doubleword store,
/// fenced on both sides, so a hart walking concurrently reads either the old leaf
/// or a table saying the same thing.
...
/// addresses nobody asked about — so [`view::Error::Absent`] rather than a
/// silent over-revoke.
...
///
/// The kernel's own walk consults the firmware map, since it maps device windows
/// too. A view only ever holds RAM, so its leaves are write-back by construction
/// and [`resident`] stays answerable long after boot info is gone.
struct ViewWalk {
    root: *const u64,
    top: usize,
}

/// Descends `root` to the leaf entry translating `address`, with its level. The
/// two walks in this file differ only in what they make of it.
fn descend(root: *const u64, top: usize, address: u64) -> Option<(u64, usize)> {
...
impl PageWalk for ViewWalk {
    fn leaf(&self, address: u64) -> Option<Leaf> {
        let (entry, level) = descend(self.root, self.top, address)?;
        let rights =
            PageProtection::new(entry & PTE_R != 0, entry & PTE_W != 0, entry & PTE_X != 0);
        Some(Leaf::at(level as u32, address, rights.cached(Cache::WriteBack), (entry >> 10) << 12))
    }
}
...
pub fn verify_owned_mapping(boot_info: &BootInfo<'_>) -> Result<(), PlatformError> {
    ...
    // The probe goes before the declared mappings are audited. Nothing else
    // references it, and the boot hart is the only one that could have cached
    // the translation the fence retires.
    let _ = unmap_leaf(state.root, state.top, probe_va, 0);
    // SAFETY: the cleared leaf is in memory, and this hart runs alone.
    unsafe { asm!("sfence.vma", options(nostack)) };
    outcome
}
...
///
/// Every address in every usable range, one leaf at a time, so the answer cannot
/// come from a lucky probe: an unmapped page inside a declared range stops the
/// walk rather than being stepped over on the way to the gigapage next door.
...
/// mode's lower canonical half, since [`init`] builds the tree before it knows
/// which mode the hart takes. Not identity-mapped: a driver that reaches a
/// device by guessing its physical address has not been given a capability.
...
impl PageWalk for TableWalk<'_> {
    fn leaf(&self, address: u64) -> Option<Leaf> {
        ...
        Some(Leaf::new(address & !(span - 1), span, protection(entry, self.inventory)))
    }
}
...
fn map_range(
    ...
    let end = align_up(end, PAGE_4K).ok_or_else(address_error)?;
    while va < end {
...
///
/// One gigapage TLB entry covers what 262 144 pages would, which is what keeps
/// a program walking a hundred gigabytes out of the page walker. Nothing here
/// splits a leaf later, so a range that only *starts* aligned still gets small
/// leaves at its ends.
