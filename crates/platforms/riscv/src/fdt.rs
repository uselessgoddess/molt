...
impl<'dtb> DeviceTree<'dtb> {
    ...
    ///
    /// `Ok(None)` is a tree that ended without an answer. A node is finished the
    /// moment a child opens as well as at `END_NODE`, because properties precede
    /// subnodes and a `cpu` node carries an interrupt controller of its own.
        ...
        for _ in 0..MAX_TOKENS {
            ...
            match token {
                ...
                PROP => {
                    ...
                    visitor.property(name(strings, nameoff)?, value, &mut cells[depth - 1]);
                }
                END_NODE => {
                    if let Some(found) = visitor.close() {
                        return Ok(Some(found));
...
/// is the answer. Token order, bounds, and the cell widths a parent declares for
/// its children are the same question every time, so they are asked once, in
/// [`DeviceTree::walk`].
trait Visit<'dtb> {
    ...
    /// Offers one property of the open node, which may declare `children`.
...
impl<'dtb> Visit<'dtb> for Node<'dtb> {
    /// A matching node either describes a window or is malformed; both end the
    /// walk, because a second host bridge would not make the first one right.
