...
impl MappingLog {
    const fn new() -> Self {
        // A zero-length placeholder — `MappedRange::new` needs constant inputs
        // and no invalid variant is available, so the placeholder is just a
        // range that covers no addresses. `len` is what says which are real.
...
/// Reads the live tables back and checks every declared range obeys W^X.
///
/// Unlike a probe of a handful of addresses, the audit walks every page of
/// every mapped range, so a stray writable leaf hiding between two correct
/// probes cannot slip through. It also refuses coarser leaves for the image
/// sections, because a megapage over `.text` would necessarily cover
/// `.rodata` with the wrong rights.
...
/// Granularity a range accepts: image sections must stay on 4 KiB leaves so
/// each one holds only its own rights, free RAM may use megapages.
...
fn map_range(
    ...
    while va < end {
        let level =
            if granularity == Granularity::LargeOk && va % PAGE_2M == 0 && end - va >= PAGE_2M {
                1
            } else {
                0
            };
