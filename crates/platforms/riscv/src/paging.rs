...
///
/// Only the address is ever read; the symbols mark boundaries and have no
/// meaningful contents of their own.
macro_rules! bound {
    ($symbol:ident) => {
...
/// Maps one private page with W^X-checked permissions and round-trips a value.
...
/// Identity-maps `[start, end)` with `rights`, using megapages where possible.
///
/// `rights` carries only R/W/X; the accessed bit is always set because nothing
/// here tracks page age, and the dirty bit follows writability, so the hardware
/// never has to fault just to update them.
fn map_range(
    root: *mut u64,
...
/// Installs one leaf at `level`, allocating the tables above it as needed.
///
/// `flags` carries the permission and status bits without `PTE_V`.
fn map_leaf(
    root: *mut u64,
    frames: &mut FrameAllocator<'_>,
    va: usize,
