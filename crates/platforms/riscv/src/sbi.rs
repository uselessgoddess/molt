...
//!
//! The console is the one service with two implementations. `console_putchar`
//! is the legacy call: one `ecall` per byte, and its return value is reserved,
//! so a firmware that dropped the byte says so nowhere. The debug console
//! extension (DBCN) replaces it with a whole-buffer write that reports an
//! error and a byte count. DBCN is only guaranteed by SBI v2.0, so [`probe`]
//! asks the base extension for it and the legacy call stays as the fallback
//! for older firmware.
