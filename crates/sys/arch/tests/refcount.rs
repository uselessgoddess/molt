...
/// A table with `runs` record slots, over storage it outlives.
fn leaves(runs: usize) -> Leaves<'static> {
    Leaves::over(vec![Run::EMPTY; runs].leak())
}

#[test]
fn hundred_gigabytes_cost_one_record() -> Result<(), Error> {
    let mut leaves = leaves(8);
