...
(Written as a Rust enum for readability. The wire form is eight little-endian
`u64`s — an id, a `u32` tag in the low half of the second, three payload words,
and a reserved tail — so the layout is stated rather than derived, and an
operation that does not use a payload word must leave it zero.)
