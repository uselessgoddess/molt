...
The MVP is not POSIX-compatible, production-secure, dynamically updateable, or
a complete microkernel. There are no processes or user/kernel transitions after
boot, so “microkernel” is less precise than Theseus's term **cytokernel**: small
cells in one address space and privilege level.
