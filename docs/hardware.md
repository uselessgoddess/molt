...
The smoke runner requires `BOOT_MARKERS` on both architectures and an
`arch_markers` list on top of it per architecture — both in
[`xtask/src/main.rs`](../xtask/src/main.rs), where the RISC-V list is what pins
the SBI console, the `satp` mode, the RAM top and the gigapage. On real silicon a
board would have to produce, for real, everything under:
