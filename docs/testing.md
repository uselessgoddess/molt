...
silent hang rather than a reported exit status; the flag appears only in
`run_qemu_interactive`, where a person is watching. Theseus and Redox both do a
version of this — Theseus
...
outside as a test that generates nothing. So each was checked by putting one bug
back into the code under it, an edit a refactor could make by accident rather
than a `panic!` planted to be found. All six failed, each on the assertion it
was written for — the stress run on the holder count eight cores hold at once.
It is the rule both sections rest on: a passing test that makes its own inputs
is a claim about the test, not about the code, until something has been seen to
fail it.
