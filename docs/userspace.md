...
That the reservations work at all was measured rather than assumed. Building one
crate twice — once for the stock target, once for one that reserves `s2`/`s5` —
and counting the instructions that name those registers:
