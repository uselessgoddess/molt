//! What a frame off the wire is allowed to do to the parsers.
//!
//! Every one of these reads a length or an option chain out of bytes a driver
//! copied from a device, and every one of them borrows the input rather than
//! copying it. So the property is containment: whatever a parser hands back
//! points inside what it was given, and a packet it accepted is one it can
//! write out and read again unchanged.
...
/// Which parser the bytes go to.
///
/// One target per protocol would spend most of its budget rediscovering that a
/// frame for one parser is refused by the others; naming the parser in the
/// input costs a byte and keeps every corpus entry pointed at one of them.
...
enum Frame<'a> {
    ...
    Ipv4(&'a [u8]),
    Ipv6(&'a [u8]),
    /// ICMPv6 refuses anything whose checksum does not cover the addresses it
    /// arrived between, and a fuzzer will not guess one. `checked` asks the
    /// harness to compute it, which is what a real sender does — leaving the
    /// other half of the inputs to prove the refusal.
