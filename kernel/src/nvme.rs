...
pub fn smoke<P: Platform>(boot_info: &BootInfo<'_>, platform: &mut P) {
    ...
    while let Some(function) = bus.function() {
        ...
        if class.class() == MASS_STORAGE && class.subclass() == NVM && class.interface() == NVME {
            target = Some(function);
        } else if function.vendor() == VIRTIO_VENDOR && function.device() == VIRTIO_IOMMU {
    ...
    let iommu_transport = Transport::probe(&iommu_function).expect("the NVMe IOMMU is modern");
    ...
    let iommu_command = iommu_function.command().expect("the NVMe IOMMU command register");
    iommu_function
        .set_command(
            iommu_command
                .with(Command::MEMORY)
        ...
        .expect("NVMe IOMMU decode and DMA authority");
    ...
    let iommu_arena = Arena::claim(&mut allocator, offset, IOMMU_TAG, &mut iommu_slots)
        .expect("contiguous frames for the NVMe IOMMU queues");
    ...
    .expect("the NVMe IOMMU completes its handshake");
    iommu.attach(endpoint).expect("NVMe endpoint attaches while quiesced");
    ...
    let mut iommu = nvme.reset().expect("the NVMe controller stops and unmaps its pages");
    function.set_command(quiesced).expect("NVMe bus mastering stays off after reset");
    assert!(iommu.poll_faults().expect("the NVMe fault queue remains valid").is_none());
    iommu.detach(endpoint).expect("the empty NVMe domain detaches");
    iommu.reset().expect("the NVMe IOMMU stops after its endpoint");
    iommu_function
        .set_command(
            iommu_command
                .with(Command::MEMORY)
        ...
        .expect("NVMe IOMMU bus mastering stays off after reset");
