//! Running the benches, and keeping what they said.
//!
//! Criterion writes its numbers for humans and for its own next run; neither
//! survives a checkout. This turns one run into a record the history branch
//! keeps and [`crate::site`] draws.
...
const PACKAGES: [&str; 2] = ["molt-core", "molt-exec"];
...
pub struct Commit {
    pub hash: String,
    pub subject: String,
    /// ISO 8601, so sorting the records by string sorts them by time.
    pub date: String,
}
...
pub struct Bench {
    pub id: String,
    pub median: f64,
    /// Median absolute deviation: the spread a comparison must clear.
    pub spread: f64,
    pub mean: f64,
    pub low: f64,
    pub high: f64,
...
/// Benches every package and writes the run beside the other build artefacts.
...
fn estimates(dir: &Path) -> Result<Bench, String> {
    #[derive(Deserialize)]
    struct Named {
        /// The group and function joined by a slash, unlike `directory_name`,
        /// which flattens the slash away for the filesystem.
        full_id: String,
    }
    #[derive(Deserialize)]
    struct Estimates {
...
/// What the commit is labelled with wherever the run is shown.
...
/// Two runs from different machines are two baselines, not a comparison.
fn machine() -> Machine {
...
/// What the run says on its own, for the terminal that asked for it.
...
/// One bench in both runs, or in only one of them.
struct Row {
    id: String,
    was: Option<f64>,
    now: f64,
    delta: Option<f64>,
    /// Past the spread and the floor both: worth a reader's attention.
    loud: bool,
}

/// The comparison a pull request reads, as GitHub-flavoured markdown.
