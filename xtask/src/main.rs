...
fn build_images() -> Result<Images, String> {
    ...
    if !kernel.is_file() {
        return Err(format!(
            "kernel binary was not created at {}",
            kernel.display()
        ));
    }
    ...
    let images = Images {
        bios: image_dir.join("molt-bios.img"),
        uefi: image_dir.join("molt-uefi.img"),
    };
...
fn smoke_test() -> Result<(), String> {
    ...
    if !serial.contains(BOOT_MARKER) {
        return Err(format!(
            "QEMU exited without the {BOOT_MARKER} serial marker"
        ));
    }
    Ok(())
}
...
fn require_qemu_success(status: ExitStatus) -> Result<(), String> {
    if status.code() == Some(QEMU_SUCCESS) {
        ...
        Err(format!(
            "QEMU reported {:?}; expected debug-exit status {QEMU_SUCCESS}",
            status.code()
        ))
    }
}
